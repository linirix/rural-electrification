...
impl Game {
    ...
    pub fn apply_decision(&mut self, decision: Decision) -> Result<String, String> {
        ...
        match decision {
            ...
            Decision::IssueStock { amount } => {
                ...
                // Exponential dilution penalties: small raises stay cheap, large raises get punished.
                let issue_discount = (0.03
                    + pressure * 0.05
                    + pressure_squared * 0.08
                ...
                let underwriting_cost = amount
                    * (0.025
                        + pressure * 0.012
                        + pressure_squared * 0.020
            ...
            Decision::BuyBackStock { amount } => {
                ...
                self.player.stock_price =
                    (theoretical_price * (1.0 + confidence_lift)).max(1.0);
                Ok(format!(
            ...
            Decision::Acquire { competitor_index } => {
                ...
                // Integration losses intentionally shave per-unit absorption so that buying scale
                // slightly worsens per-customer capacity until the buyer invests to catch up.
            ...
            Decision::AdjustRate { delta_cents } => {
                if delta_cents > 0.0 && self.rate_frozen() {
                    return Err(
                        "The regulator's rate freeze blocks any rate increase right now."
                            .to_string(),
                    );
                }
            ...
            Decision::Maintenance { spend } => {
                ...
                self.require_cash(spend)?;
                self.player.cash -= spend;
                let gain = (spend / 22_000.0)
                    * (1.05_f64 - self.player.reliability).max(0.05);
                self.player.reliability =
                    (self.player.reliability + gain).clamp(0.35, 0.98);
                self.player.reputation =
    ...
    pub fn advance_quarter(&mut self) -> QuarterReport {
        ...
        // Snapshot pre-churn customer counts so next quarter's competitor_plans can detect losses.
    ...
    pub fn acquisition_price(&self, competitor_index: usize) -> f64 {
        ...
        // Non-linear: each step toward monopoly costs more than the last.
    ...
    fn shock_active(&self, predicate: impl Fn(&ShockKind) -> bool) -> bool {
        self.active_shocks.iter().any(|shock| predicate(&shock.kind))
    }
    ...
    fn advance_shocks(&mut self, events: &mut Vec<String>) {
        ...
        if self.rng.chance(0.045) && !self.shock_active(|kind| matches!(kind, ShockKind::RateFreeze)) {
            self.active_shocks.push(ActiveShock {
                kind: ShockKind::RateFreeze,
                quarters_remaining: 2 + (self.rng.next_u64() % 2) as u32,
            });
            events.push(
                "Regulators imposed a temporary rate freeze across the territory.".to_string(),
            );
        }
            ...
            })
        {
            ...
            events.push(
                "A regional slowdown cut new connection prospects sharply.".to_string(),
            );
        }
    ...
    fn maybe_spawn_startup(&mut self, events: &mut Vec<String>) {
        ...
        let conditions_favor_entrant = (consolidated || high_rates) && unmet_demand;
        if !conditions_favor_entrant {
            return;
        }
    ...
    fn competitor_plans(&mut self, events: &mut Vec<String>) {
        ...
        for index in 0..competitor_count {
            ...
            let lost_share = competitor.last_quarter_customers > 0.0
                && share_now < share_last - 0.005;
            ...
            } else if player_premium_pricing && near_capacity && competitor.rate_cents < 13.4 {
            ...
            if lost_share && competitor.cash > 4_500.0 {
                ...
                competitor.reputation =
                    (competitor.reputation + spend / 5_500.0).clamp(0.0, 100.0);
                events.push(format!(
                ...
                competitor.reputation =
                    (competitor.reputation + spend / 7_000.0).clamp(0.0, 100.0);
            }
            ...
            if competitor.reliability < 0.78 && competitor.cash > 3_500.0 {
                ...
                competitor.cash -= spend;
                let gain = (spend / 22_000.0)
                    * (1.05_f64 - competitor.reliability).max(0.05);
                competitor.reliability =
                    (competitor.reliability + gain).clamp(0.35, 0.98);
            }
        }
    }
    ...
    fn update_stock_price(&mut self, finances: &FirmFinances) {
        ...
        // Book equity reflects the balance sheet: assets in the ground plus cash, minus debt.
        ...
        let new_price = reverted * (1.0 + macro_signal + noise);
        self.player.stock_price = new_price.max(1.0);
    }
...
fn settle_utility(
    ...
    // Infrastructure ages each quarter; maintenance is needed to hold reliability.
...
fn shock_expired_message(kind: &ShockKind) -> String {
    match kind {
        ShockKind::RateFreeze => "The regulator's rate freeze expired.".to_string(),
...
mod tests {
    ...
    fn stock_issuance_has_no_fixed_proceeds_cap() {
        ...
        // No hard cap: very large issues still go through and net substantial cash,
        // even if exponential dilution penalties claim a meaningful share.
        assert!(game.player.cash > starting_cash + 60_000.0);
    ...
    fn excessive_dilution_yields_less_cash_per_dollar() {
        ...
        assert!(
            ...
            "small yield {} should beat large yield {} by at least 10c per dollar",
            small_yield,
            large_yield
        );
    }
    ...
    #[test]
    fn high_share_makes_acquisitions_exponentially_pricier() {
        let mut low = Game::with_seed(50);
        ...
        assert!(
            ...
            "consolidation premium should rise non-linearly: low {}, high {}",
            low_price,
            high_price
        );
    }
    ...
    fn acquisition_absorbs_target_cash_and_debt() {
        ...
        game.apply_decision(Decision::Acquire { competitor_index: 2 })
            .unwrap();

        // Player gets ~80% of target's cash absorbed.
        assert!(game.player.cash > starting_cash_after_payment_only + target.cash * 0.5);
        // Player assumes most of target's debt.
    ...
    fn reliability_decays_without_maintenance() {
        ...
        assert!(
            ...
            "reliability should decay over 6 quarters without maintenance: {} -> {}",
    ...
    fn maintenance_has_diminishing_returns_at_high_reliability() {
        ...
        assert!(
            ...
            "low reliability should gain much more from the same spend: low {} vs high {}",
            low_gain,
            high_gain
        );
    }
    ...
    fn stock_price_reflects_balance_sheet_equity() {
        ...
        // Run the same number of quarters so noise is comparable.
        for _ in 0..3 {
        ...
        assert!(
            ...
            "healthier balance sheet should produce a higher stock price: healthy {} vs leveraged {}",
    ...
    fn cost_shock_increases_operating_cost() {
        let mut normal = Game::with_seed(92);
        let mut shocked = Game::with_seed(92);
        ...
        assert!(
            ...
            "input cost shock should raise operating cost noticeably: normal {} vs shocked {}",
    ...
    fn startup_can_enter_consolidated_high_rate_market() {
        ...
        // Force conditions: player dominates, rates are high, demand is unserved.
        ...
        // Run several quarters; with favorable conditions and 10-16% per-quarter chance,
        // a startup should appear with high probability inside this window.
        for _ in 0..30 {
            game.advance_quarter();
    ...
    fn competitors_react_to_player_undercutting() {
        ...
        // Seed the prior-quarter snapshot so churn shows up as lost share for rivals.
        for competitor in &mut game.competitors {
            competitor.last_quarter_customers = competitor.customers;
        }

        let starting_rates: Vec<f64> =
            game.competitors.iter().map(|c| c.rate_cents).collect();
        ...
        assert!(
            any_cut,
            "expected at least one competitor to cut rates in response to player undercut"
        );
    }
